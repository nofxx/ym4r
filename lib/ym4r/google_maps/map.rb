  ...
  module GoogleMaps 
    ...
    class GMap
      ...
      def control_init(controls = {})
        @init << add_control(GSmallMapControl.new).to_s if controls[:small_map]
        @init << add_control(GLargeMapControl.new).to_s if controls[:large_map]
        @init << add_control(GSmallZoomControl.new).to_s if controls[:small_zoom]
        @init << add_control(GScaleControl.new).to_s if controls[:scale]
        @init << add_control(GMapTypeControl.new).to_s if controls[:map_type]
        @init << add_control(GOverviewMapControl.new).to_s if controls[:overview_map]
      end
      ...
      def icon_init(icon , variable)
        @global_init << icon.declare(variable).to_s
      end
